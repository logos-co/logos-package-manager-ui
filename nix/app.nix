...
pkgs.stdenv.mkDerivation rec {
  ...
  preFixup = ''
    ...
    export QML_IMPORT_PATH="${pkgs.qt6.qtdeclarative}/lib/qt-6/qml:${pkgs.qt6.qtbase}/lib/qt-6/qml"
    ...
    find $out -type f -executable -exec sh -c '
      if file "$1" | grep -q "ELF.*executable"; then
        ...
        if echo "$1" | grep -q "/logos-package-manager-ui-app$"; then
  ...
  configurePhase = ''
    ...
    echo "Configuring logos-package-manager-ui-app..."
  ...
  buildPhase = ''
    ...
    cmake --build build
    echo "logos-package-manager-ui-app built successfully!"
  ...
  installPhase = ''
    ...
    if [ -f "build/bin/logos-package-manager-ui-app" ]; then
      cp build/bin/logos-package-manager-ui-app "$out/bin/"
      echo "Installed logos-package-manager-ui-app binary"
    fi
    ...
    # Copy capability_module plugin into the modules directory (must be loaded first)
    if [ -f "${logosCapabilityModule}/lib/capability_module_plugin.$OS_EXT" ]; then
      ...
      echo "Installed capability_module_plugin.$OS_EXT"
    fi
    ...
    if [ -f "${logosPackageManagerUI}/lib/package_manager_ui.$OS_EXT" ]; then
      ...
      echo "Installed package_manager_ui.$OS_EXT"
    fi

    # Create a README for reference
    cat > $out/README.txt <<EOF
Logos Package Manager UI App - Build Information
=================================================
...
Runtime Layout:
- Binary: $out/bin/logos-package-manager-ui-app
- Libraries: $out/lib
- Modules: $out/modules
- Qt Plugin: $out/package_manager_ui.$OS_EXT

Usage:
  $out/bin/logos-package-manager-ui-app
EOF
