...
LogosBadge {
    ...
    LogosToolTip {
        text: {
            ...
            if (root.modelData.isVariantAvailable !== true)
                return d.notAvailableText(root.modelData)
            if ((root.modelData.installStatus | 0) === PackageManagerUi.Failed)
                return root.modelData.errorMessage || ""
            return ""
        }
